...
func UpdateRankingEntries(categoryId string, subCategoryId string, gameId string) (err error) {
	...
	case "eventLocationCompletion":
		query = "SELECT ?, ?, RANK() OVER (ORDER BY COUNT(DISTINCT COALESCE(el.title, pel.title)) / aec.count DESC), 0, a.uuid, COUNT(DISTINCT COALESCE(el.title, pel.title)) / aec.count, aect.maxTimestamp FROM eventCompletions ec JOIN accounts a ON a.uuid = ec.uuid LEFT JOIN eventLocations el ON el.id = ec.eventId AND ec.type = 0 LEFT JOIN playerEventLocations pel ON pel.id = ec.eventId AND ec.type = 1 JOIN (SELECT COUNT(DISTINCT COALESCE(ael.title, apel.title)) count FROM eventCompletions aec LEFT JOIN eventLocations ael ON ael.id = aec.eventId AND aec.type = 0 LEFT JOIN playerEventLocations apel ON apel.id = aec.eventId AND aec.type = 1 JOIN gameEventPeriods agep ON agep.id = COALESCE(ael.gamePeriodId, apel.gamePeriodId) AND agep.game = ? WHERE (ael.title IS NOT NULL OR apel.title IS NOT NULL)) aec JOIN (SELECT aect.uuid, MAX(aect.timestampCompleted) maxTimestamp FROM eventCompletions aect GROUP BY aect.uuid) aect ON aect.uuid = ec.uuid JOIN gameEventPeriods gep ON gep.id = COALESCE(el.gamePeriodId, pel.gamePeriodId) AND gep.game = ? JOIN eventPeriods ep ON ep.id = gep.periodId"
